...
export default function DriverPanel() {
  ...
  // Cuando carga el driver, preseleccionar el primer método disponible
  useEffect(() => {
    if (!driver) return;
    const activeTypes = (driver.payment_methods || [])
      .filter(m => m.is_active)
      .map(m => m.type);
    const firstAvailable = PAYMENTS.find(p => !activeTypes.includes(p.value));
    setNewType(firstAvailable?.value || '');
  }, [driver]);
  ...
  const addPayment = async () => {
    if (!driver || !newType) return;

    // Verificar si ya existe ese método activo
    const exists = driver.payment_methods?.some(m => m.type === newType && m.is_active);
    if (exists) return;
  ...
  // Métodos disponibles para agregar (los que no están activos aún)
  const activeTypes = (driver?.payment_methods || [])
    .filter(m => m.is_active)
    .map(m => m.type);
  const availableToAdd = PAYMENTS.filter(p => !activeTypes.includes(p.value));

  if (loading) return <div style={{ padding: 32, textAlign: 'center', color: '#555' }}>Cargando panel...</div>;
  if (!driver)  return <DriverSetupForm onComplete={loadDriver} />;
      ...
      {activeTab === 'status' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 14 }}>
              ...
              <div>
                <div style={{ fontSize: 22, letterSpacing: 2 }}>
                  <span style={{ color: '#F5C000' }}>{'★'.repeat(Math.round(driver.avg_rating || 0))}</span>
                  <span style={{ color: '#333' }}>{'★'.repeat(5 - Math.round(driver.avg_rating || 0))}</span>
                </div>
          ...
          <div style={{ background: '#1A1A1A', border: '0.5px solid #2a2a2a', borderRadius: 14, padding: 20 }}>
            ...
            {/* Agregar — solo si hay métodos disponibles */}
            {availableToAdd.length > 0 ? (
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button className="btn btn-primary" onClick={addPayment}
                  disabled={addingPayment || !newType}
                  style={{ padding: '9px 16px', fontSize: 13 }}>
                  {addingPayment ? '...' : 'Agregar'}
                </button>
              </div>
            ) : (
              <div style={{ fontSize: 13, color: '#22c55e', textAlign: 'center', padding: '8px 0' }}>
                ✅ Tienes todos los métodos de pago activos
              </div>
            )}
          </div>
        </div>
